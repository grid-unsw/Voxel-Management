...
public class PointOctreeNode<T> {
    ...
    public void GetColliding(ref Bounds checkBounds, List<T> result)
    {
        ...
        for (int i = 0; i < objects.Count; i++)
        {
            ...
            if (checkBounds.Contains(currentObject.Pos))
            {
                result.Add(currentObject.Obj);
            }
        }
    ...
    public void GetColliding(ref Bounds checkBounds, List<Vector3> result)
    {
        // Are the input bounds at least partially in this node?
        if (!bounds.Intersects(checkBounds))
        {
	...
	/// <param name="result">List result.</param>
	/// <returns>Objects that intersect with the specified bounds.</returns>
	public void GetFirstInside(ref Bounds checkBounds, ref T result)
    {
        // Are the input bounds at least partially in this node?
        if (!bounds.Intersects(checkBounds))
        {
        ...
        if (children != null )
        {
            for (int i = 0; i < 8; i++)
            {
                children[i].GetFirstInside(ref checkBounds, ref result);
            }
        }
    }
