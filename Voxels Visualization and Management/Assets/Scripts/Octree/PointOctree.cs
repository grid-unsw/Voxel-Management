...
public class PointOctree<T> {
    ...
    public void GetCollidingPositions(List<Vector3> collidingWith, Bounds checkBounds)
    {
        //#if UNITY_EDITOR
        // For debugging
        //AddCollisionCheck(checkBounds);
	...
	/// Returns an array of objects that intersect with the specified bounds, if any. Otherwise returns an empty array. See also: IsColliding.
	/// </summary>
	/// <param name="inside">first object inside the bounding box.</param>
	/// <param name="checkBounds">bounds to check.</param>
	/// <returns>Objects that intersect with the specified bounds.</returns>
	public void GetFirstInside(ref T inside, Bounds checkBounds)
    {
        //#if UNITY_EDITOR
        // For debugging
        //AddCollisionCheck(checkBounds);
        //#endif
        rootNode.GetFirstInside(ref checkBounds, ref inside);
    }
